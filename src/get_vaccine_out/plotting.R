...
rt_two_by_one_scenario_plot <- function(scenarios, scenario_objects, fit, end_date) {
  ...
  rt_list[[length(rt_list) + 1]] <- squire.page:::get_Rt(fit)
  rt_list[[length(rt_list)]]$scenario <- unique(scenarios$Rt)[1]
  rt_list[[length(rt_list)]]$Vaccine <- "Baseline"

  for(i in seq_along(unique(scenarios$Rt))[-1]) {
    rt_list[[length(rt_list) + 1]] <- rt_list[[length(rt_list)]]
    rt_list[[length(rt_list)]]$scenario <- unique(scenarios$Rt)[i]
  }
...
combine_plot_outputs <- function(vacc_plot, rt_plot, death_plot, death_averted_plot){
  ...
  title <- cowplot::ggdraw() +
    cowplot::draw_label(
      paste0(fit$parameters$country, " (", iso3c, ") - 100 Days Mission Scenarios"),
      fontface = 'bold',
      x = 0.5
    )
...
weekly_death_alt_plot <- function(dih, fit){
  ...
  dih %>%
    ...
    group_by(date) %>%
    summarise(q025 = quantile(deaths, 0.025, na.rm=TRUE),
              q25 = quantile(deaths, 0.25, na.rm=TRUE),
              q50 = quantile(deaths, 0.5, na.rm=TRUE),
              q75 = quantile(deaths, 0.75, na.rm=TRUE),
              q975 = quantile(deaths, 0.975, na.rm=TRUE)) %>%
    ggplot(aes(date, q50)) +
    ggplot2::theme_bw() +
    ggplot2::theme(legend.position = "none",
                   axis.title.x = ggplot2::element_blank()) +
    ...
    geom_ribbon(aes(ymin = q025,ymax=q975, x = date), fill = "#408da7", alpha = 0.25)  +
    #geom_ribbon(aes(ymin = q25,ymax=q75, x = date), fill = "#408da7", alpha = 0.75)  +
    geom_line(aes(y = q50, x = date), color = "#408da7", lwd = 1)  +
    ggtitle(paste0("Baseline Fit: ", fit$parameters$country, " (",data$iso[1], ")"))
