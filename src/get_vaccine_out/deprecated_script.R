...
if(commit %in%  c("32d334d96937536e0ac9da1d8c37f12841c88af5", "825b1e9e14c1dedbe1e6f3cd7ecdc86ea55b1f9c")) {

  fit_old <- grab_fit(iso3c, excess_mortality, booster, "2e5ecf5b1b15698ff4d3b1adcb07d95aa81bc4ed")

  prim_dat <- fit_old$parameters$tt_primary_doses + fit_old$inputs$start_date
  new_tt <- c(0, prim_dat[-1] - fit$inputs$start_date)

  fit$parameters$tt_primary_doses <- new_tt
  fit$parameters$tt_booster_doses <- new_tt
