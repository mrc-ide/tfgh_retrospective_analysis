...
implement_target_Rt.rt_optimised <- function(fit, iso3c, force_opening, dih) {
  ...
  if(!is.na(open_date)){
    ...
    rt_new <- lapply(seq_along(rt), function(i){
      ...
      rt_open <- quantile(x2$Rt, prob = c(rt_quant), na.rm=TRUE)
...
plot_deaths <- function(scenario_df, facet = FALSE){
  ...
  if(facet) {
    ...
    ggplot(baseline, aes(x = as.integer(date - cepi_start_date) + 100)) +
      ...
      geom_line(
        data = scenarios_x  %>% mutate(Vaccine = as.character(Vaccine)),
        aes(x = as.integer(date - cepi_start_date) + 100, y = deaths_med, colour = Vaccine, group = scenario),
        inherit.aes = FALSE,
        lwd = 0.75
      ) +
...
calculate_openness <- function(fit, scenario_objects, end_date){
  ...
  full_openness_rt <- simple_Rt(fit) %>%
    map_dbl(~quantile(.x$Rt[.x$date < as.Date("2022-01-01") & .x$date > as.Date("2020-07-01")], rt_quant))
...
# adapt vaccine pre data
preextend_vacc <- function(fit) {
  ...
  extend <- function(t,x){
    ...
    return(list("t" = tnew, "x" = xnew))
  }

  prim <- extend(fit$parameters$tt_primary_doses,
                 fit$parameters$primary_doses)

  boost <- extend(fit$parameters$tt_booster_doses,
                  fit$parameters$booster_doses)

  fit$parameters$tt_primary_doses <- prim$t
  fit$parameters$primary_doses <- prim$x
  fit$parameters$tt_booster_doses <- boost$t
  fit$parameters$booster_doses <- boost$x

  return(fit)

}

get_vacc_data <- function(scenarios, scenario_objects, fit, combine = TRUE, end_date) {
  ...
  vacc <- df %>%
    group_by(Vaccine) %>%
    mutate(primary = cumsum(primary),
           booster = cumsum(booster)) %>%
    complete(date = seq.Date(min(date), max(df$date), 1)) %>%
    fill(primary:booster, .direction = "down") %>%
